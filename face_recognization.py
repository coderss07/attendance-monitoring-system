...
class Face_recognition:
    def __init__(self, root):
        ...
        img1 = img1.resize((1400, 670), Image.ANTIALIAS)
    ...
    def take_attendance(self, var1, var2, var3, var4, var5):
        with open("attendance/attendance.csv", "r+", newline = '\n') as f:
            ...
            if((var1 not in name_list) and (var2 not in name_list) and (var3 not in name_list)):
                curr = datetime.now()
                d1 = strftime("%d/%m/%Y")
    ...
    def recognize_face(self):
        def draw_rect(img, classifier, scl_fac, minNeighbour, col, txt, cls):
            ...
            for (x, y, w, h) in feature:
                ...
                db = mysql.connector.connect(host = "localhost", username = "sarthu", password = "sharma78", database = "attendance_system")
                my_cursor = db.cursor()
                ...
                if conf > 77:
                    cv2.putText(img, f"Student Id: {var1}", (x, y - 60), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 0, 255), 2)
                    cv2.putText(img, f"Name: {var3}", (x, y - 35), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 0, 255), 2)
                    cv2.putText(img, f"Course: {var2}", (x, y - 10), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 0, 255), 2)
